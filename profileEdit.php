<?php 
	...
	if(isset($_POST['screenName'])) {
		if(!empty($_POST['screenName'])) {
			$screen_name = $getFromU->checkInput($_POST['screenName']);
			$profile_bio = $getFromU->checkInput($_POST['bio']);
			...
			else if (strlen($profile_bio) > 120) {
				$error = "Description must be under 120 characters";		
			}
			else if (strlen($country) > 80) {
				$error = "Country name is too long";		
			}
			else {
				$getFromU->update('users', $user_id, array('screen_name' => $screen_name, 'bio' => $profile_bio, 'country' => $country, 'website' => $website));
